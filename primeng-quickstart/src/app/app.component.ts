...
export class AppComponent implements OnInit, OnDestroy {
  ...
  private readonly viewport = inject(ViewportScroller); // for page-level scrolling
  ...
  // When sorting, PrimeNG often emits onPage after onSort. We wait for that
  // and only load once to avoid a duplicate (canceled) request.
  private awaitingSortPage = false;
  ...
  private selectListItem(item: ListItemModel): void {
    ...
    // Scroll the selected item into view within the listbox (and the page if needed)
    setTimeout(() => this.scrollToSelectedListItem(), 0);
  }
  ...
  onPage(event: any) {
    ...
    // Single place that actually fetches data for both paging and sort→page flows
    this.loadTableData();

    // If we were waiting for onPage due to a sort, consume the flag
    this.awaitingSortPage = false;
  }
  ...
  onSort(event: any) {
    ...
    // Show overlay immediately so users see feedback
    this.loadingRows.set(true);

    // PrimeNG usually triggers onPage to reset to first page after sorting.
    // Use that as the single fetch trigger to avoid duplicate requests.
    this.awaitingSortPage = true;
    this.pageIndex.set(0);
    if (this.dataTable) {
      (this.dataTable as any).first = 0; // triggers onPage
    }

    // Fallback: if onPage doesn't fire (edge cases), do exactly one manual load.
    setTimeout(() => {
      if (this.awaitingSortPage) {
        this.awaitingSortPage = false;
        this.loadTableData();
      }
    }, 50);
  ...
  private saveCurrentTableWidths(): void {
    ...
    const tableEl =
      host.querySelector<HTMLElement>('.p-table-scrollable-header-table') ?? host.querySelector<HTMLElement>('table');
    if (!tableEl) return;
  ...
  private scrollToSelectedListItem(): void {
    const id = this.anchorIdForCurrentSelection();
    if (!id) return;
    ...
    // 2) If the element is outside the browser viewport (left panel off-screen), bring the page to it.
